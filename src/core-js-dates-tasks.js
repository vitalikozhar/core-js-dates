...
function getDayName(date) {
  ...
  const dayAbbr = d.toUTCString().slice(0, 3);
...
function getNextFriday(date) {
  ...
  const nameOfDay = copyDate.getUTCDay();
  const daysAdd = (5 - nameOfDay + 7) % 7 || 7;
  return new Date(copyDate.setUTCDate(copyDate.getUTCDate() + daysAdd));
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const firstDay = Date.parse(dateStart);
  const secondDay = Date.parse(dateEnd);
  const msInDay = 1000 * 60 * 60 * 24;
  return 1 + Math.abs(firstDay - secondDay) / msInDay;
}
...
function formatDate(date) {
  const copyDate = new Date(date);
  return copyDate.toLocaleString('en-US', {
    month: 'numeric',
    day: 'numeric',
    year: 'numeric',
    hour: 'numeric',
...
function getCountWeekendsInMonth(month, year) {
  ...
  do {
    const newDate = new Date(year, newMonth, i);
    counter += newDate.getDay() === 0 || newDate.getDay() === 6 ? 1 : 0;
    i += 1;
  } while (i <= days);
  return counter;
}
